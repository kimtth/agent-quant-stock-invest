...
charts are rendered with `blessed-contrib`, and every advisory panel is produced
by a **Microsoft Agent Framework workflow** running as a companion Python
process. The source lives in [chart-cli](../chart-cli); every command below is
...
`/backtest` loads its own daily history — two years by default, or the period
given as the argument after the symbol (`30d`, `6mo`, `1y`, `5y`, or `max` for
everything the data source has). Its natural-language request is handed to the Agent Framework
`signal_generator`, which authors Python for the repository's validated
research REPL. The REPL must produce aligned `BuySignal`, `SellSignal`, and
`Description` rows before the CLI backtests them. A chat provider is required;
chart-cli does not replace the request with a predefined signal.

The output streams progress, then shows the requested objective, the generated
strategy and its rationale, return, CAGR, drawdown, Sharpe ratio, win rate,
exposure, recent fills, and a research-only verdict.
...
The final example is a signal-generation request: the agent writes and the REPL
validates the rule implementation before it is backtested. The period must
leave at least 30 valid price bars. Results exclude fees, slippage, and taxes,
and are historical research rather than a trading recommendation.
...
| `/model offline` | Detach the model and use the deterministic rule strategy. |
...
| `backtest.py` | The simulation engine the backtest workflow drives. |
...
newline-delimited JSON: a request `{"kind": ..., "payload": ...}` in, one
response out. Responses are prefixed with `@@REPORT@@ ` so ordinary log output
is ignored, and their `kind` field (`advisory`, `ask`, `model`) routes them back
to the caller.
...
- **`offline`** — no provider is configured, or an agent call fails. The same
  graphs run with a transparent SMA/RSI crossover strategy and
  volatility-scaled position caps, so the dashboard stays usable. The status
  bar shows `AGENT:RULES` and the `NOTES` panel shows `mode offline`.
