"""Native Semantic Kernel plugins for agent-led investment research."""
...
from .research_repl import DATASET_SIGNALS, DATASET_STOCK, ResearchPythonRepl
...
class StockDataPlugin:
    ...
    @kernel_function(description="Fetch historical OHLCV data as the signal REPL input CSV.")
    def fetch_stock_data(self, ticker: str, start_date: str, end_date: str) -> str:
        try:
            ...
            path = self.output / DATASET_STOCK
            frame.to_csv(path, index=False)
            return f"SUCCESS: Saved {len(frame)} rows to {path}."
        except Exception as error:
            return f"ERROR: {error}"


class PythonReplPlugin:
    """Expose the shared development-only REPL as a Semantic Kernel function."""

    def __init__(self, output: Path) -> None:
        self.output = output
        self.repl = ResearchPythonRepl(output)

    @kernel_function(
        description=(
            "Execute agent-authored pandas/numpy/ta code that reads INPUT_PATH and writes "
            "a validated stock_signals.csv to OUTPUT_PATH."
        )
    )
...
class BacktestingPlugin:
    ...
    def backtest_strategy(self, capital: float = 10_000) -> str:
        try:
            frame, signals = (
                pd.read_csv(self.output / DATASET_STOCK),
                pd.read_csv(self.output / DATASET_SIGNALS),
            )
            if len(frame) != len(signals):
                return "ERROR: Price and signal rows must align before backtesting."
            ...
            metric_file.write_text(
                "\n".join(
                    f"{key}: {value}" for key, value in metrics.model_dump().items()
                ),
                encoding="utf-8",
            )
...
class ReportingPlugin:
    ...
    def plot_performance(self) -> str:
        try:
            ...
            figure, (returns, drawdown) = plt.subplots(
                2, 1, figsize=(12, 8), sharex=True
            )
            returns.plot(dates, frame["CumulativeReturns"])
            drawdown.fill_between(
                dates, frame["Drawdown"], 0, alpha=0.35, color="crimson"
            )
